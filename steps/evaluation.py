...
class Model_Evaluation:
    ...
    def main(self):
        ...
        dataset = self.load_qa_dataset(QA_DATASET_PATH)
        questions = [item["question"] for item in dataset]
        references = [item["answer"] for item in dataset]

        print("Generating answers...")
        predictions = self.generate_answers(pipeline, dataset, self.MAX_NEW_TOKENS)

        print("Evaluating...")
        scores = self.evaluate_outputs(predictions, references)
